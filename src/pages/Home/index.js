...
const Home = () => {
    const { items } = useSelector(selectCoffeeItems);
    const proposedItems  = items.filter(item => item.recommended === true);
    ...
    useEffect(() => {
        ...
        const visibleItems = filterPost(searchItems(data, term), filter);
